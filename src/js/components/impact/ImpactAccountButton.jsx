...
/** Item ID, type, and friendly type-name for the thing we're sharing */
const getShareProps = (brand, campaign) => {
	if (campaign) return [campaign.id, C.TYPES.Campaign, 'Campaign'];
	if (brand) return [brand.id, C.TYPES.Advertiser, 'Brand'];
	return [];
};
...
const ImpactShareLine = ({style, className, brand, campaign}) => {
	// not-logged in can't share and pseudo users can't reshare
	if (!Login.getId() || Login.getId().endsWith('pseudo')) return null;

	const [id, type, typeName] = getShareProps(brand, campaign);
	if (!id) return null;

	const shareId = shareThingId(type, id);
	const pvItem = getDataItem({type, id, status: KStatus.PUBLISHED});
	const name = `Dashboard for ${typeName} ${pvItem.value?.name || id}`;
	const noEmails = !DataStore.getUrlValue('listemails');

	const swProps = {name, className, style, shareId, noEmails};

	return <ShareWidget hasButton hasLink {...swProps } />;
};


const AccountMenuItem = ({itemIconClass, itemText, itemUrl, active}) => {
	if (itemText.length > 280) itemText = itemText.slice(0, 280);
	const thumbnail = <div className={space('impact-icon', (itemIconClass || 'placeholder-thumbnail'))} />;
...
const ImpactAccountButton = ({curMaster, curSubBrand, curCampaign, noShare}) => {
	...
	assert(Login.isLoggedIn()); // this shouldn't even be loaded if you're not logged in! We should have moved to the sign in before this!
	...
	const modalProps = { id: 'ads-for-good-modal', prioritized: true, headerClassName: 'red-top-border noClose' };
	const accountOnClick = () => openAndPopulateModal({ ...modalProps, Content: accountContent });
	const shareOnClick = () => openAndPopulateModal({ ...modalProps, Content: shareContent });
