...
const QuartersCard = ({baseFilters}) => {
	...
	console.log("pvsBuckets", pvsBuckets);
	pvsBuckets.forEach((pvBucket, i) => {
		if ( ! pvBucket.value) return;

		console.log("pvBucket", pvBucket);
		...
		let bucket = pvBucket.value.total;
		if ( ! bucket || ! bucket.length) {
			return; // no data for this quarter
		}

		// Display kg or tonnes?
		let thisCarbon = getSumColumnEmissions(bucket, 'totalEmissions');
...
const insertCampaignData = (prevProps, campaign, table) => {
	...
	data.splice(insertIndex, 0, getSumColumnEmissions(table, 'totalEmissions'));
